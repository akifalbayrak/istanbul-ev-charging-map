...
const MapButton = memo(({ onClick }: { onClick: () => void }) => (
  <button
    onClick={onClick}
    className="flex items-center justify-center w-10 h-10 bg-white/90 hover:bg-white text-gray-800 rounded-lg shadow-lg backdrop-blur-sm transition-all duration-300 hover:shadow-xl cursor-pointer"
...
const FeatureHighlight = memo(({ icon, text, gradient, textColor, iconColor }: {
  icon: string;
  text: string;
  gradient: string;
  textColor: string;
  ...
  <div className={`group flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r ${gradient} rounded-lg shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5`}>
    <svg className={`w-4 h-4 sm:w-5 sm:h-5 ${iconColor} group-hover:scale-110 transition-transform`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={icon} />
    </svg>
    <span className={`text-xs sm:text-sm font-medium ${textColor}`}>{text}</span>
  </div>
));
    ...
    return (
      <main className="h-screen relative">
        ...
        <div className="absolute bottom-4 left-4 z-[1000]">
          <MapButton onClick={handleReturnHome} />
        </div>
      </main>
    );
  }
